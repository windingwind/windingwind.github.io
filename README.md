...
- **Blog posts** — add markdown files to [`content/blog/`](content/blog/). Frontmatter supports `title`, `date` (`YYYY-MM-DD`), and optional `description` (used as the preview excerpt; defaults to the first paragraph). Each post is published at `/blog/<filename>/` and all posts appear as previews in the homepage Blog section (newest first). Build logic lives in [`scripts/blog.js`](scripts/blog.js).
...
npm run lint     # ESLint + Stylelint + Prettier check (also runs in CI)
